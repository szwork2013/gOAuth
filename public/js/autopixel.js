...
function detect(ua){
    ...
    os.weixin = ua.match(/MicroMessenger/i);
}
...
(function fit() {
	...
	if (screenwidth < 360) {
		viewport.content = "width=640,target-densitydpi=" + viewPortdpi + ",initial-scale=" + viewPortScale + ",minimum-scale=" + viewPortScale + ",maximum-scale=" + viewPortScale + ",user-scalable=no"
	} else {
		if (window.os ? window.os.ios : ai.ovb.ios()) {
			...
			var ratio = screenwidth / 640;
			viewport.content = "width=640,target-densitydpi=" + viewPortdpi + ",initial-scale=0.999,minimum-scale=0.999,maximum-scale=0.999,user-scalable=no"
		}
	}
