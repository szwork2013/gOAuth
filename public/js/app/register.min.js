...
function init() {
	...
	$validators.init({
				...
				if (issuccess) {
					...
					$.ajax({
						...
						success: function(data) {
							// 显示验证结果
							if (data.errcode) {
								$("input[name='mobile']").parent().addClass("err").find(".lbl_err").html("* 该手机号码已被注册");
							}
						}
					});
				}
		...
		password: {
			...
			stringLength: {
				min: 6,
				max: 20,
				message: '* 设置密码长度只允许输入6到20位'
			},
            regular: {
                reg: /^\w{6,20}$/,
                message: '* 设置密码只能包含数字字母下划线'
            }
		},
		vcode: {
			notEmpty: {
	...
	// 选中条款
	$(".lbl_agreement .fa").click(function() {
	...
	$(".btn_register").click(function() {
		// 触发是否勾选条款
		if ($(".lbl_agreement .fa").hasClass("fa-check-square-o")) {
			// 验证触发
			if ($validators.start()) {
				// 是否准入
				if (isbool) {
					...
					$.ajax({
						...
						data: {
							...
							isregister: $(".lbl_agreement .fa").hasClass("fa-check-square-o")
						},
						success: function(data) {
							// 可以准入
							isbool = true;
							...
							switch(data.errcode) {
			                    ...
			                    case 402:
				                    $("body").css("overflow", "hidden");
									$(".app_alert .modal-title").html("对不起！");
									$(".app_alert .modal-body").html("手机号码已存在");
									$(".app_alert .confirm").hide();
									$("#btnGb").html("确定");
									$(".app_alert").addClass("warning").show();
			                    break;
			                   	...
			                   	case 403:
			                   		$("body").css("overflow", "hidden");
									$(".app_alert .modal-title").html("对不起！");
									$(".app_alert .modal-body").html("openid已绑定其他手机号");
									$(".app_alert .confirm").hide();
									$("#btnGb").html("确定");
									$(".app_alert").addClass("warning").show();
			                   	break;
			                   	...
			                   	case 404:
			                   		$("body").css("overflow", "hidden");
									$(".app_alert .modal-title").html("对不起！");
									$(".app_alert .modal-body").html("注册失败");
									$(".app_alert .confirm").hide();
									$("#btnGb").html("确定");
			                   	...
			                   	case 498:
			                   		$("body").css("overflow", "hidden");
									$(".app_alert .modal-title").html("对不起！");
									$(".app_alert .modal-body").html("尝试登录失败");
									$(".app_alert .confirm").hide();
									$("#btnGb").html("确定");
									$(".app_alert").addClass("warning").show();
			                   	break;
			...
			$("body").css("overflow", "hidden");
			$(".app_alert .modal-title").html("对不起！");
			$(".app_alert .modal-body").html("* 请阅读并同意《车信网用户协议》");
			$(".app_alert .confirm").hide();
			$("#btnGb").html("确定");
			$(".app_alert").addClass("warning").show();
		}
