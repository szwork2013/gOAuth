...
exports.getAccessToken = function(req, callback) {
    if ($.config.debug) {
        // 是否过期
        if ($.config.wx.access_token_time && (new Date()).getTime() < $.config.wx.access_token_time) {
            callback(null, $.config.wx.access_token);
        } else {
            // 获取凭证接口
            $.ajax({
                    ...
                    (function (data) {
                        if (data && data.errcode) {
                            ...
                            $.config.wx.access_token = data.access_token;
                            $.config.wx.access_token_time = (function (miao) {
                                // 当前时间
                                var t = new Date();
                                // 转换成毫秒
                                var t_s = t.getTime();
        ...
        // 获取凭证接口
        $.ajax({
                url: "{0}/api/WeChatCallback/GetToken/{1}".format(
                    $.config.wechatUrl,
                    $.config.wx.appid
                ),
                type: "get",
                ...
                callback(err, JSON.parse(data).errmsg ? JSON.parse(data).errmsg.token : "");
            }
        );
    }
};
